...
// @version      1.6.1
// @description  Restored summary table logic + 3D Printing Highlights + Department Settings (GS/Systems/BYO) + Perfect Nav Button Alignment + Auto Hours Worked (EST) from first sale time + Last Sale time
// @author       grant
// @match        */Sales/Retail*
// @grant        none
// ==/UserScript==
...
(function() {
    ...
    // Compute auto hours: first-sale time floored to the hour, current EST time floored to 10 min.
    ...
    function runCalculator() {
        ...
        // Find the first sale (bottom of the list), fetch its detail page time, and autofill Hours Worked.
        function autofillHoursWorked() {
            ...
            const txMatch = bottomRow.textContent.match(/\d{3}-[A-Z]{2}-\d{8}/);
            if (!txMatch) return;
            const txId = txMatch[0];

            // Visual hint while loading
            const prevTitle = hoursInput.title;
            hoursInput.title = 'Auto-filling from first sale (' + txId + ')...';

            fetchTransactionTime(txId).then(timeStr => {
                hoursInput.title = prevTitle || '';
                if (!timeStr) return;

                const autoHours = computeAutoHours(timeStr);
                if (autoHours === null) return;

                // Don't stomp on the user if they've already typed into the field.
                if (document.activeElement === hoursInput) return;

                hoursInput.value = autoHours.toFixed(2);
                hoursInput.title = 'Auto: first sale ' + timeStr + ' (EST)';

                // Trigger recalculation through the existing pipeline.
                calculateAll();
            });
        }
