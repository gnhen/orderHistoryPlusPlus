...
(function() {
    ...
    const OH_SP_STORAGE_KEY = 'ohpp_salesperson_names';

    function loadSalespersonNames() {
        try {
            const raw = localStorage.getItem(OH_SP_STORAGE_KEY);
            if (raw) {
    ...
    function saveSalespersonNames(names) {
        try { localStorage.setItem(OH_SP_STORAGE_KEY, JSON.stringify(names)); }
    ...
    function rebuildSalespersonOptions(select, names) {
        ...
        manageOpt.textContent = '✎ Edit list…';
    ...
    function injectSalespersonDropdown() {
        ...
        select.title = 'Quick-pick a sales person (editable list)';
        ...
        select.addEventListener('change', () => {
            ...
            if (val === '__add__') {
                const entry = prompt('Add a sales person name:');
                if (entry && entry.trim()) {
                    const clean = entry.trim();
                    if (!names.includes(clean)) {
                        names.push(clean);
                        saveSalespersonNames(names);
                        rebuildSalespersonOptions(select, names);
                    }
                    setSalesPersonValue(clean);
                    select.value = clean;
                } else {
                    select.value = '';
                }
            ...
            if (val === '__manage__') {
                const edited = prompt(
                    'Edit the sales person list (comma-separated):',
                    names.join(', ')
                );
                if (edited !== null) {
                    const arr = edited.split(',').map(s => s.trim()).filter(Boolean);
                    names = arr.length ? arr : ['person1', 'person2'];
                    saveSalespersonNames(names);
                    rebuildSalespersonOptions(select, names);
                }
                select.value = '';
