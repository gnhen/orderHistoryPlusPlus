...
// @version      1.18.0
// @description  Summary table logic + 3D Printing Highlights (bulletproof category classifier) + Department Settings (GS/Systems/BYO; Systems pays every item the selected rate except service plans which are always 10%) + Nav Button Alignment + Auto Hours Worked (EST): live for today / first->last sale for past days, first sale skips return-only transactions + Last Sale time + Sales Person quick-pick dropdown + Monthly Sales nav button (black/cornflower theme, OrderHistory++ customer count, negative revenue, full category breakdown + top 3 SKUs by quantity + 3D Printer Attach plan rate counted per printer unit, no clipboard) + Clear button defaults Date to today and Store to 071 - Sharonville instead of blank (fixes "Store field is required" after Clear) + peripherals/mice keep native orange (no yellow override on OH++ activation) + Change Commission Rate mode: mark items as Out of department for flat 1% commission, saved locally per person + BYO key item is CPU-only (GPU only via highest-price) + removes dead Product Reservation nav item + GS commission brackets by unit price applied per item (unit x qty) + click Customers Served to toggle counting all transactions vs excluding return-only transactions + saved Department and Systems commission rate per assigned-to person (stored locally per browser)
// @author       grant
// @match        */Sales/Retail*
// @grant        none
// ==/UserScript==
...
(function() {
    ...
    // True ONLY for an actual 3D printer MACHINE (not filament, nozzles, hotends,
    // build plates, AMS units, resin, or any other consumable/part). Used for the
    // "3D Printer Attach" metric so plan attach is measured against printer sales.
    function ohIsActual3DPrinter(rawDesc) {
        const d = ' ' + String(rawDesc || '').toLowerCase() + ' ';
        const t = d.replace(/[^a-z0-9.\/+ ]+/g, ' ').replace(/\s+/g, ' ');

        // "3D Printer Filament/Spool" uses "3D Printer" as an adjective for a
        // consumable, so disqualify that pattern up front.
        if (/\b3d printer (filament|spool|nozzle|build plate|bed)\b/.test(t)) return false;

        // An explicit machine noun phrase wins outright: "... 3D Printer",
        // "FDM Printer", "Resin Printer", etc. A bundled AMS or the word "resin"
        // does NOT disqualify these — they're still printer machines.
        const machinePhrase = /\b(3d printer|fdm printer|fff printer|sla printer|msla printer|resin printer)\b/;
        if (machinePhrase.test(t)) return true;

        // Otherwise, disqualify anything that is plainly a consumable/part.
        const partOrConsumable = /\b(filament|filaments|spool|spools|nozzle|nozzles|hotend|hot end|\bhe\b|extruder|extruders|build plate|build plates|pei plate|pei sheet|\bpei\b|bed plate|heatbed|heated bed|ams|automatic material system|poop chute|purge|gantry|carbon rod|lidar|pla|pla\+|petg|pet-g|\babs\b|\basa\b|tpu|tpe|pva|hips|resin)\b/;
        if (partOrConsumable.test(t)) return false;

        // Bare machine wording with no consumable context (e.g. "FDM", "3D print").
        const machineWords = /\b(3d printing|3d print|\bfdm\b|\bfff\b|\bsla\b|msla)\b/;
        return machineWords.test(t);
    }
    ...
    // return-only transactions excluded), and every line item's category/SKU.
    // It then reports grand totals plus a full category breakdown and the
    // top 3 SKUs by quantity. No clipboard is used.
    // ---------------------------------------------------------------------
    function runMonthlySales() {
        ...
        async function run() {
            ...
            const skuStats = {}; // sku -> { qty, desc } (whole month, by quantity)
            // Per-transaction UNIT counts for the 3D Printer Attach metric:
            // tx -> { printerQty, planQty }. Within a transaction, successful
            // attaches = min(printerQty, planQty); the denominator is total
            // printer units. Printers ONLY (not filament/nozzles/parts).
            const txPrinterPlan = {};
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                if (data) {
                    ...
                    for (const it of data.items) {
                        // 3D Printer Attach tracking by unit (printers ONLY).
                        if (it.tx) {
                            if (!txPrinterPlan[it.tx]) txPrinterPlan[it.tx] = { printerQty: 0, planQty: 0 };
                            const units = Math.max(0, it.qty); // ignore returns (negative qty)
                            if (ohIsActual3DPrinter(it.desc)) txPrinterPlan[it.tx].printerQty += units;
                            else if (ohClassifyCategory(it.desc) === 'Service Plans') txPrinterPlan[it.tx].planQty += units;
                        }

                        // Tally categories by item AND SKUs by quantity. Service Plans
                        // are attach-on protection plans rather than merchandise, so
                        // they're left out of both rankings.
                        ...
                        if (it.sku) {
                            if (!skuStats[it.sku]) skuStats[it.sku] = { qty: 0, desc: it.desc };
                            // Count net units sold (returns subtract). Keep a
                            // human-readable description for display.
                            skuStats[it.sku].qty += it.qty;
                            if (it.desc) skuStats[it.sku].desc = it.desc;
                        }
                    }
                }
            ...
            // 3D Printer Attach: per transaction, successful attaches are
            // min(printer units, plan units); summed over the month and divided
            // by total printer units sold.
            let printerUnits = 0, attachedUnits = 0;
            for (const tx in txPrinterPlan) {
                const p = txPrinterPlan[tx].printerQty;
                const pl = txPrinterPlan[tx].planQty;
                if (p > 0) {
                    printerUnits += p;
                    attachedUnits += Math.min(p, pl);
                }
            }
            const printerAttachPct = printerUnits > 0
                ? Math.round((attachedUnits / printerUnits) * 100)
                : null;

            // Full category breakdown, sorted by item count (desc).
            const categoryRanking = Object.keys(categoryCounts)
                .map(cat => ({
                    cat: cat,
                    count: categoryCounts[cat],
                    pct: categorizedItemTotal > 0
                        ? Math.round((categoryCounts[cat] / categorizedItemTotal) * 100)
                        : 0
                }))
                .sort((a, b) => b.count - a.count);

            // Top 3 SKUs by net quantity sold (ignore zero/negative net).
            const topSkus = Object.keys(skuStats)
                .map(sku => ({ sku: sku, qty: skuStats[sku].qty, desc: skuStats[sku].desc }))
                .filter(s => s.qty > 0)
            ...
            if (topSkus.length === 0) {
                ...
                topSkus.forEach((s, idx) => {
                    log(`   ${idx + 1}. ${s.sku} x${s.qty}`);
                    log(`      ${s.desc.slice(0, 40)}`);
                });
            }

            // 3D Printer Attach: per-unit plan attach rate on printers sold.
            if (printerAttachPct === null) {
                log(`\uD83D\uDDA8\uFE0F 3D Printer Attach: N/A (no printers sold)`);
            } else {
                log(`\uD83D\uDDA8\uFE0F 3D Printer Attach: ${printerAttachPct}% (${attachedUnits}/${printerUnits})`);
            }
            log("\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550");
        }
    ...
    // Per-item "out of department" commission overrides (1% flat), persisted
    // locally per browser and scoped to the active salesperson. Keyed by a
    // stable per-line id: "txId|line|sku".
    const OH_OOD_PREFIX = 'ohpp-ood:';
    let oodOverrides = new Set(); // active in-memory set of out-of-dept line keys

    function loadOodOverrides(name) {
        oodOverrides = new Set();
        if (!name) return;
        try {
            const raw = localStorage.getItem(OH_OOD_PREFIX + name.toLowerCase());
            if (raw) {
                const arr = JSON.parse(raw);
                if (Array.isArray(arr)) oodOverrides = new Set(arr);
            }
        } catch (e) { /* localStorage unavailable: start empty */ }
    }

    function saveOodOverrides(name) {
        if (!name) return;
        try {
            localStorage.setItem(OH_OOD_PREFIX + name.toLowerCase(), JSON.stringify([...oodOverrides]));
        } catch (e) { /* localStorage unavailable: skip persistence */ }
    }

    // Build the stable line key for a transaction row from its cells.
    function oodRowKey(row) {
        const cells = row.querySelectorAll('td');
        if (cells.length === 0) return null;
        const txMatch = row.textContent.match(/\d{3}-[A-Z]{2}-\d{8}/);
        const tx = txMatch ? txMatch[0] : 'unknown';
        const line = (cells[2]?.textContent.trim() || '');
    ...
    function runCalculator() {
        ...
        const css = `
            ...
            #oh-rate-mode-btn { margin-top: 10px; margin-bottom: 10px; margin-left: 8px; padding: 6px 12px; background-color: #6495ED; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; }
            #oh-rate-mode-btn.active { background-color: #2f5fc4; }
            ...
            body.oh-rate-mode-active .transaction-row { cursor: pointer; }
            body.oh-rate-mode-active .transaction-row:hover { background-color: #d6e4ff !important; outline: 2px solid #6495ED; }
            .transaction-row.oh-removed-item { opacity: 0.3; text-decoration: line-through; background-color: #f8f9fa !important; }
            .transaction-row.oh-ood-item { outline: 2px dashed #6495ED; outline-offset: -2px; }
            #oh-rate-popup-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 1000000; display: flex; align-items: center; justify-content: center; }
            #oh-rate-popup { background: #fff; color: #111; padding: 18px; border-radius: 8px; min-width: 280px; box-shadow: 0 4px 20px rgba(0,0,0,0.4); font-family: sans-serif; }
            #oh-rate-popup h3 { margin: 0 0 6px 0; font-size: 15px; }
            #oh-rate-popup .oh-rate-desc { font-size: 12px; color: #555; margin-bottom: 14px; word-break: break-word; }
            #oh-rate-popup .oh-rate-btns { display: flex; gap: 10px; }
            #oh-rate-popup button { flex: 1; padding: 9px; border: none; border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 13px; }
            #oh-rate-popup .oh-rate-default { background: #e9ecef; color: #111; }
            #oh-rate-popup .oh-rate-default.selected { background: #28a745; color: #fff; }
            #oh-rate-popup .oh-rate-ood { background: #e9ecef; color: #111; }
            #oh-rate-popup .oh-rate-ood.selected { background: #6495ED; color: #fff; }
            #oh-rate-popup .oh-rate-cancel { margin-top: 12px; width: 100%; background: transparent; color: #888; font-weight: normal; }
        `;
        ...
        loadOodOverrides(getCurrentSalesPersonName());
        ...
        function injectHeaderItemsButton() {
            ...
            if (detailsElements.length > 0) {
                ...
                btn.onclick = () => {
                    ...
                    if (headerModeActive) { rateModeActive = false; syncRateModeBtn(); }
                ...
                // Blue "Change Commission Rate" button to the right of it.
                const rateBtn = document.createElement('button');
                rateBtn.id = 'oh-rate-mode-btn';
                rateBtn.type = 'button';
                rateBtn.textContent = 'Change Commission Rate';
                rateBtn.onclick = () => {
                    rateModeActive = !rateModeActive;
                    if (rateModeActive) {
                        headerModeActive = false;
                        document.body.classList.remove('oh-header-mode-active');
                        btn.textContent = 'Enable Header Items Mode';
                        btn.classList.remove('active');
                    }
                    syncRateModeBtn();
                };
                btn.parentNode.insertBefore(rateBtn, btn.nextSibling);
            }
        }

        function syncRateModeBtn() {
            const rateBtn = document.getElementById('oh-rate-mode-btn');
            document.body.classList.toggle('oh-rate-mode-active', rateModeActive);
            if (rateBtn) {
                rateBtn.textContent = rateModeActive ? 'Done Changing Rates' : 'Change Commission Rate';
                rateBtn.classList.toggle('active', rateModeActive);
            }
        }

        // Popup to set an item's commission rate (Default vs Out of department).
        function openRatePopup(row) {
            const key = oodRowKey(row);
            if (!key) return;
            const cells = row.querySelectorAll('td');
            const desc = (cells[4]?.textContent.trim() || 'this item');

            const overlay = document.createElement('div');
            overlay.id = 'oh-rate-popup-overlay';

            const isOod = oodOverrides.has(key);
            overlay.innerHTML =
                '<div id="oh-rate-popup">' +
                '<h3>Commission Rate</h3>' +
                '<div class="oh-rate-desc"></div>' +
                '<div class="oh-rate-btns">' +
                '<button class="oh-rate-default' + (!isOod ? ' selected' : '') + '">Default</button>' +
                '<button class="oh-rate-ood' + (isOod ? ' selected' : '') + '">Out of department</button>' +
                '</div>' +
                '<button class="oh-rate-cancel">Cancel</button>' +
                '</div>';
            // Set description as text (avoid HTML injection from product names).
            overlay.querySelector('.oh-rate-desc').textContent = desc;

            const close = () => overlay.remove();
            overlay.addEventListener('click', (e) => { if (e.target === overlay) close(); });

            overlay.querySelector('.oh-rate-default').onclick = () => {
                oodOverrides.delete(key);
                row.classList.remove('oh-ood-item');
                saveOodOverrides(getCurrentSalesPersonName());
                calculateAll();
                close();
            };
            overlay.querySelector('.oh-rate-ood').onclick = () => {
                oodOverrides.add(key);
                row.classList.add('oh-ood-item');
                saveOodOverrides(getCurrentSalesPersonName());
                calculateAll();
                close();
            };
            overlay.querySelector('.oh-rate-cancel').onclick = close;
        ...
        function attachTransactionRowListeners() {
            document.querySelectorAll('tr').forEach(row => {
                if (row.textContent.match(/\d{3}-[A-Z]{2}-\d{8}/)) {
                    ...
                    // Mark rows that already have a saved out-of-department override.
                    const key = oodRowKey(row);
                    if (key && oodOverrides.has(key)) row.classList.add('oh-ood-item');
                    row.onclick = (e) => {
                        if (rateModeActive) {
                            openRatePopup(row);
                        } else if (headerModeActive) {
        ...
        function calculateAll() {
            let visibleRevenue = 0, calculatedCommissionTotal = 0, calcSpRevenue = 0, calcSpQty = 0;
            ...
            document.querySelectorAll('.transaction-row').forEach(row => {
                row.classList.remove('oh-r-red', 'oh-r-yellow', 'oh-r-green');
                if (row.classList.contains('oh-removed-item')) return;
                ...
                const oodKey = `${txId}|${cells[2]?.textContent.trim() || ''}|${cells[3]?.textContent.trim() || ''}`;
                ...
                // CPUs are the only automatic BYO key item. GPUs are detected
                // separately and only become key items via the highest-price
                // fallback (Priority 2), never automatically.
                const isCpu = /\bcpu\b|processor|intel\s+core|ryzen/.test(descCellStr) &&
                    !/\bgpu\b|graphics card|video card|geforce|radeon/.test(descCellStr);

                if (isServicePlan) {
                    row.classList.add('oh-r-red');
                    calcSpRevenue += totalVal;
                    calcSpQty += qtyVal;
                    // In Systems, every item is paid at the selected rate EXCEPT
                    // service plans, which are always a flat 10% and never change.
                    if (currentDept === 'Systems') {
                        calculatedCommissionTotal += totalVal * 0.10;
                    }
                    return;
                }

                // Peripherals/mice keep the page's native orange (no oh-r-yellow override).
                if (is3DPrint) row.classList.add('oh-r-green');

                // Out-of-department override: this item commissions at a flat 1%,
                // regardless of department, and is excluded from BYO islands.
                if (oodOverrides.has(oodKey)) {
                    row.classList.add('oh-ood-item');
                    calculatedCommissionTotal += totalVal * 0.01;
                    return;
                }
                ...
                if (currentDept === 'GS') {
                    ...
                    transactionIslands[txId].push({ totalVal: totalVal, isCpu: isCpu });
                }
            });
            ...
            if (currentDept === 'BYO') {
                for (let txId in transactionIslands) {
                    ...
                    // Priority 1: the highest-priced CPU is the automatic key item.
                    let maxCpuPrice = -Infinity;
                    for (let i = 0; i < items.length; i++) {
                        if (items[i].isCpu && items[i].totalVal > maxCpuPrice) {
                            maxCpuPrice = items[i].totalVal;
                            keyItemIndex = i;
                        }
                    }

                    // Priority 2: with no CPU, the overall highest-priced item is
                    // the key item (this is the only way a GPU becomes key).
                    if (keyItemIndex === -1) {
    ...
    // including the Date box and the Store dropdown. We want Clear to leave the
    // Date set to TODAY and the Store set back to "071 - Sharonville" (value 71)
    // instead of empty. (An empty Store makes the next Search fail with "The
    // Store field is required.") We let the native clear run, then immediately
    // re-set both fields and keep their Kendo widgets in sync.
    // ---------------------------------------------------------------------
    ...
    // Restore the Store dropdown to the default store 071 - Sharonville (value 71).
    function setStoreToDefault() {
        const storeInput = document.getElementById('StoreId');
        if (!storeInput) return;
        const DEFAULT_STORE = '71';
        storeInput.value = DEFAULT_STORE;
        if (window.jQuery) {
            const widget = window.jQuery(storeInput).data('kendoDropDownList');
            if (widget && typeof widget.value === 'function') {
                widget.value(DEFAULT_STORE); // sets selected value + visible text
                // If the store list hasn't loaded yet, the widget may not resolve
                // the text immediately; re-apply once its dataSource is ready.
                if (widget.value() !== DEFAULT_STORE && widget.dataSource &&
                    typeof widget.dataSource.fetch === 'function') {
                    widget.dataSource.fetch(() => widget.value(DEFAULT_STORE));
                }
            }
            window.jQuery(storeInput).trigger('change');
        } else {
            storeInput.dispatchEvent(new Event('change', { bubbles: true }));
        }
    }

    // Re-apply the fields the native Clear wipes that we want to keep populated.
    function restoreClearedDefaults() {
        setDateToToday();
        setStoreToDefault();
    }
    ...
    function injectClearButtonOverride() {
        ...
        // Run AFTER the native clear handler so our values aren't wiped. A microtask
        // + rAF defers us past the native click handler and any Kendo updates.
        clearBtn.addEventListener('click', () => {
            Promise.resolve().then(() => requestAnimationFrame(restoreClearedDefaults));
        });
    }
